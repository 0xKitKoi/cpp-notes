...
int main() {
   ...
   for (i = 0; i < zipCodeState.size(); ++i) {
		...
		// ZipCodeState stores the zipcode and state abreviatons.
		// abbrevState stores the state abbreviation and state name to match
		// statePopulation stores the state name and the population in the state.
		if (zip == zipCodeState[i].GetKey()) {
	...
	for (i = 0; i < abbrevState.size(); ++i) {
		...
		if (abbrev == abbrevState[i].GetKey()) {
	...
	for (i = 0; i < statePopulation.size(); ++i) {
		...
		if (state == statePopulation[i].GetKey()) {
